...
public class ConnectFour extends JFrame {
   ...
   public boolean hasWon(Seed theSeed, int rowSelected, int colSelected) {
      // Check for 4-in-a-line on the rowSelected (horizontal)
      int count = 0;
      for (int col = 0; col < COLS; ++col) {
      ...
      count = 0;
      for (int d = -3; d <= 3; d++) {
         int row = rowSelected + d;
         int col = colSelected + d;
         if (row >= 0 && row < ROWS && col >= 0 && col < COLS && board[row][col] == theSeed) {
            ++count;
            if (count == 4) return true;  // found
         } else {
            count = 0; // reset
   ...
   public void updateGame(Seed currentPlayer, int row, int col) {
      ...
      if (hasWon(currentPlayer, row, col)) {
         ...
         for (int r = 0; r < ROWS; r++) {
            ...
            if (!isDraw) break; // Exit outer loop if not a draw
         }
         currentState = isDraw ? State.DRAW : State.PLAYING; // Set state to DRAW or continue playing
      }
   }
